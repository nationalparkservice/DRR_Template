...
RRpackages <- c("markdown", "rmarkdown", "pander", "knitr", "yaml", "devtools")
...
dp_flags <- QCkit::get_dp_flags(directory = path, force=TRUE)

#calculate Unweighted Response Rates:
dp_flags$RRU<-(dp_flags$Cell_count - dp_flags$AE_flag - dp_flags$P_flag - dp_flags$R_flag)/dp_flags$Cell_count

#generate table:
knitr::kable(dp_flags, caption = 'Summary of data quality flags for the data package.', col.names = c("A", "AE", "R", "P", "Total", "RRU"), format = "pandoc")
...
path<-"../../DataPackageWorkflow/dataPackages/BICY_Veg_Data_Package_Example"
df_flags <- QCkit::get_df_flags(directory = path, force = TRUE)

#Calculate the Unweighted Response Rates:
df_flags$RRU <- (df_flags$Cell_count - df_flags$AE_flag - df_flags$P_flag - df_flags$R_flag)/df_flags$Cell_count

#Generate the table
knitr::kable(df_flags, caption = 'Summary of data quality flags for each data file within the data package.', col.names = c("File Name", "A", "AE", "R", "P", "Total", "RRU"), format = "pandoc")
```

```{r data_column_flagging, echo=FALSE, eval=TRUE}
#set directory to the location of your data package:
path <- "../../DataPackageWorkflow/dataPackages/BICY_Veg_Data_Package_Example"
dc_flags <- QCkit::get_dc_flags(directory = path, force = TRUE)

#Calculate the Unweighted Response Rates:
dc_flags$RRU <- (dc_flags$Cell_count - dc_flags$AE_flag - dc_flags$P_flag - dc_flags$R_flag)/dc_flags$Cell_count

#Generate the table:
knitr::kable(dc_flags, caption = 'Summary of data quality flags for each column where flagging was applied.', col.names = c("File Name", "Flagged Column", "A", "AE", "R", "P", "Total", "RRU"), format = "pandoc")
```

Unweighted Response Rates (RRU) is calculated as number of accepted (A) data values divided by the total number of values. Note that all unflagged values in addition to those flagged as "A" are considered accepted, which may appear to inflate the RRU value.
