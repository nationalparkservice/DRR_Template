...
- Do qualification codes use of numbers in lieu of codes? (This may be okay in some circumstances but numbers are generally not descriptive)
- Do codes (qualification or otherwise) mean different things for different fields?
- Do enumerated domains contain options that are not wholistic of all options (i.e., limited to only those values present within the data set as opposed to all that are applicable)?
...
- Do datasets, manifests, and metadata all clearly indicate whether a dataset contains data that are protected from release? (and therefore may be incomplete or not suitable for use for some users)

# Metadata
- Do all fields with enumerated domains have definitions for all possible (as opposed to all used) domains.
- Are all fields are of the appropriate format for the type of data they contain (numeric, string, date, logical)
- Is the metadata in either FGDC, EML, or ISO format and does it validate as such?
- Does the title conveys the “what, where, when, who, and (if applicable) scale” of the data?
- Does the abstract provides a short statement that allows users to quickly evaluate the content of the dataset?
- Is the coordinate system and datum are defined appropriately (both horizontal and vertical?
...
- Do uncensored versions of data package contain clear indications of exactly what data are to be protected from accidental release and how (and to whom) release is appropriate?
