...
# List author affiliations. The order of author affiliations must match the order of the authors in AuthorNames. Note that the entirety of each affiliation is enclosed in quotations. Line breaks are indicated with a backslash followed by two spaces(\  ). Do not worry about indentation or word wrapping.
...
`r (paste0("doi: *https://doi.org/10.38750/", DRR_DSRefID, "*"))`
...
```
```{r authors2_depricated, echo=FALSE, results="asis"}
...
```{r Listing, eval=FALSE, echo=FALSE, ref.label=knitr::all_labels()}
