...
#Author 1 affiliation and address. Note use of <br> for line breaks.
AuthorAffiliation1<-"NPS Inventory and Monitory Division<br>
1201 Oakrdige Dr, Suite 150<br>
Fort Collins, Colorado"

#Author 1 ORCID iD. If no ORCID iD, set to NA (not applicable). See https://www.orcid.org to register an ORCID. 
...
#AuthorAffiliation3<-"Author Affiliation 3"
#AuthorORCID3<-NA
...
for(i in 1:(AuthorNumb/2)){
  ...
  if(!is.na(curr$ORCID)){
    ...
    orc<-gsub('dummyhtml', paste0("https://orcid.org/", curr$ORCID), orc)
...
```{r authors_deprecated, echo=FALSE, eval=FALSE, results="asis"}
#formatting template:
template<-"#### %s
...
This DRR describes the data package `r dataPackage1Title` which contains a metadata file and `r dataPackage1_filenumb` data files. These data were compiled by the National Park Service Biological Resources Division and are available at `r dataPackage1DOI`. The files in the data package are:

```{r datapackage_files, echo=FALSE, results="asis"}
#formatting template:
template<-"**%s -** %s
...
The data within the data records listed above have been reviewed by staff in the NPS Inventory and Monitoring Division to ensure accuracy, completeness, and consistency with documented data quality standards, as well as for usability and reproducibility. The *`r dataPackage1Title`* is suitable for its intended use as of the date of processing (`r Sys.Date()`).
